...
pub fn get_repos(user: User) -> Vec<Repo> {
    ...
    let iterations = (user.public_repos / 100) + 1;

    for page in 0..iterations {
        let mut url = Url::parse(user.repos_url.clone().unwrap().as_str()).unwrap();
        let page_num = page + 1;
        let query_string = format!("per_page=100&page={}", page_num);
        let query_option = Some(query_string.as_str());
        url.set_query(query_option);
        ...
        let mut iteration_repos: Vec<Repo> = json::decode(&*response_content).unwrap();
        repos.append(&mut iteration_repos);
    }
    return repos;
}
